...
from model import FP_Model
from utils import Encode_FP
...
def train(model, loader, optimizer):
    ...
    for _, (fp, label) in enumerate(loader):
        fp = fp.to(device)
        optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default="cav", help='which dataset')
    ...
    train_set = Encode_FP(train_df.index.values, train_df,args.data)
    valid_set = Encode_FP(valid_df.index.values, valid_df,args.data)
    test_set60 = Encode_FP(test_df60.index.values, test_df60, args.data)
    test_set70 = Encode_FP(test_df70.index.values, test_df70,args.data)
    ...
    # Model
    tox_model = FP_Model().to(device)
    optim = torch.optim.Adam(tox_model.parameters(), lr=args.lr)

    loss_fn = torch.nn.BCEWithLogitsLoss()
    models = []
    val_results, t60_results, t70_results = [],[], []
    for epoch in range(args.epochs):
        print("Epoch: ", epoch + 1)
        train_loss = train(tox_model, train_loader, optim)
        ...
        t60_true, t60_prob = predicting(tox_model, test_loader60)
        res_60 = evaluate(t60_true, t60_prob)
        t60_results.append(res_60)
        print("Test60: ", res_60)

        t70_true, t70_prob = predicting(tox_model, test_loader70)
    ...
    torch.save(best_model, 'ckpts/' + args.data + '_' + str(args.rs) + '_fp.pth')
    print("Best Epoch:",best_index)

    best_t60_result,best_t70_result=t60_results[best_index],t70_results[best_index]
    pd.DataFrame([best_t60_result]).to_csv('results/' + args.data + '_t60_fp_' + str(args.rs) + '.csv',
                                           columns=['Accuracy', 'AUROC', 'AUPRC', 'Recall', 'F1', 'MCC'], index=False)

    pd.DataFrame([best_t70_result]).to_csv('results/' + args.data + '_t70_fp_' + str(args.rs) + '.csv',
                                           columns=['Accuracy', 'AUROC', 'AUPRC', 'Recall', 'F1', 'MCC'], index=False)
