...
from model import FusionCTox_Concat
...
if __name__ == '__main__':
    ...
    # Model
    tox_model = FusionCTox_Concat().to(device)
    ...
    torch.save(best_model, 'ckpts/' + args.data + '_' + str(args.rs) + '_concat.pth')
    print("Best Epoch:", best_index)
    best_t60_result, best_t70_result = t60_results[best_index], t70_results[best_index]
    pd.DataFrame([best_t60_result]).to_csv('results/' + args.data + '_t60_concat_' + str(args.rs) + '.csv',
                                           columns=['Accuracy', 'AUROC', 'AUPRC', 'Recall', 'F1', 'MCC'], index=False)

    pd.DataFrame([best_t70_result]).to_csv('results/' + args.data + '_t70_concat_' + str(args.rs) + '.csv',
                                           columns=['Accuracy', 'AUROC', 'AUPRC', 'Recall', 'F1', 'MCC'], index=False)
